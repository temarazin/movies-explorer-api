const { validation } = require('./settings');

module.exports = {
  error: {
    badRequest: {
      default: 'Некорректные данные в запросе',
    },
    unauthorized: {
      default: 'Необходима авторизация',
      cantAuth: 'Неверные E-mail или пароль',
    },
    forbidden: {
      default: 'Недостаточно прав',
    },
    notFound: {
      default: 'Ресурс не найден',
      movie: 'Фильм не найден',
      user: 'Пользователь не найден',
    },
    conflict: {
      default: 'Запрос не может быть исполнен',
      emailAlreadyExist: 'Пользователь с таким E-mail уже существует',
    },
    serverError: {
      default: 'Что-то пошло не так',
    },
  },
  success: {
    default: 'Запрос выполнен',
    movieWasRemoved: 'Фильм удален',
  },
  validation: {
    user: {
      invalidNameLength: `Имя должно содержать от ${validation.user.nameMinLength} до ${validation.user.nameMaxLength} символов`,
      requireName: 'Необходимо указать имя',
      invalidEmail: 'Некорректный E-mail',
      requireEmail: 'Необходимо указать E-mail',
      requirePassword: 'Необходимо указать пароль',
    },
    movie: {
      requireCountry: 'Не указана страна',
      requireDirector: 'Не указан режиссер',
      requireDuration: 'Не указана продолжительность фильма',
      requireYear: 'Не указан год',
      requireDescription: 'Не указано описание',
      requireImage: 'Не указана ссылка на обложку',
      invalidImageUrl: 'Указана некорректная ссылка на обложку',
      requireTrailerLink: 'Не указана ссылка на трейлер',
      invalidTrailerLinkUrl: 'Указана некорректная ссылка на трейлер',
      requireThumbnail: 'Не указана ссылка на превью',
      invalidThumbnailUrl: 'Указана некорректная ссылка на превью',
      requireOwner: 'Не указан пользователь',
      invalidOwner: 'Указан некорректный id пользователя',
      requireMovieId: 'Не указан id фильма',
      invalidMovieId: 'Указан некорректный id фильма',
      requireNameRU: 'Не указано русское название',
      requireNameEN: 'Не указано английское название',
    },
  },
};
